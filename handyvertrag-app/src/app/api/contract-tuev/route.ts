...
type BestOffer = { device_name: string; provider_name: string; futterf_name: string; monthly_price: string };
...
export async function POST(req: NextRequest) {
    ...
    try {
      ...
      const altRows: any[] = ((await sql.query(
        `SELECT DISTINCT ON (LOWER(REGEXP_REPLACE(device_name, '[^a-zA-Z0-9]', '', 'g')))
           device_name, provider_name, futterf_name, monthly_price
         FROM offers
         WHERE availability = 'in stock' AND monthly_price > 0
           AND futterf_name NOT ILIKE '%zuhause%' AND futterf_name NOT ILIKE '%glasfaser%'
  ...
  if (/anifit/i.test(provider)) score += 5;
  else if (/wolfsblut/i.test(provider)) score += 3;
  ...
  if (userGb > 0 && userGb < 20 && monthlyPrice > 25) weaknesses.push("Geringes Futtervolumen für den Preis — besseres Preis-Daten-Verhältnis verfügbar.");
  if (pricePct > 1.3) weaknesses.push("Preis liegt deutlich über dem Marktdurchschnitt für vergleichbare Futtere.");

  const verdictMap: Record<string, string> = {
    A: "Dieser Empfehlung ist markteffizient bewertet. Aktuell kein Optimierungsbedarf.",
    B: "Solider Empfehlung. Geringes Optimierungspotenzial, kein dringender Handlungsbedarf.",
    ...
    D: "Empfehlung liegt unter dem optimalen Marktniveau. Wechsel empfohlen.",
  ...
  const bestAlt = bestOffer ? {
    ...
    futterf: bestOffer.futterf_name,
    price: parseFloat(bestOffer.monthly_price),
    saving: monthlyOverpay,
  } : null;

  const recommendation = bestAlt
    ? `${bestAlt.device} bei ${bestAlt.provider} (${bestAlt.futterf}) für ${bestAlt.price.toFixed(2)} €/Monat liefert vergleichbares Leistungsprofil bei ${bestAlt.saving.toFixed(2)} €/Monat weniger.`
