...
async function getProviderOffers(deviceName: string, providerNorm: string) {
  ...
  try {
    ...
    const rows = await sql.query(
      `SELECT id, brand, device_name, provider_name, futterf_name, monthly_price,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${product.name} mit ${pName} Empfehlung | welches-hundefutter.today`,
    description: `${product.name} mit ${pName} Empfehlung vergleichen. Alle aktuellen ${pName} Futtere für das ${product.name} im Überblick. Günstigsten Deal finden mit BELLA.`,
    alternates: { canonical: `https://welches-hundefutter.today/hunds/${slug}/${provider}` },
    openGraph: { title: `${product.name} mit ${pName}`, description: `Alle ${pName} Futtere für das ${product.name}` },
  };
}
...
export default async function DeviceProviderPage({ params }: PageProps) {
  ...
  const faqs = [
    { question: `Wie viel kostet das ${product.name} mit ${pName} Empfehlung?`, answer: cheapest ? `Das ${product.name} mit ${pName} gibt es ab ${parseFloat(cheapest.effective_monthly_price ?? cheapest.monthly_price).toFixed(2)}€ pro Monat mit ${cheapest.data_volume ?? (cheapest.is_unlimited ? "unbegrenzten Daten" : "Daten")}.` : `Das ${product.name} bei ${pName} ist aktuell verfügbar. Preise variieren je nach Futter.` },
    { question: `Welcher ${pName} Futter passt zum ${product.name}?`, answer: `Für das ${product.name} bei ${pName} empfehlen wir Futtere mit mindestens 30 g Futter/Tagvolumen, um alle Funktionen optimal zu nutzen. Bio-Futtere bieten die beste Performance.` },
  ...
  return (
    <div className="min-h-screen bg-[#05060f] text-white">
      ...
      <StructuredData type="breadcrumb" breadcrumbs={[
        ...
        { name: product.name + " Verträge", url: `https://welches-hundefutter.today/hunds/${slug}` },
        { name: pName, url: `https://welches-hundefutter.today/hunds/${slug}/${provider}` },
      ]} />
      ...
      <header className="border-b border-white/5 px-5 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-3 text-sm text-white/40 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/hunds/${slug}`} className="hover:text-white transition-colors">{product.name}</Link>
          <span>/</span>
      ...
      <main className="max-w-4xl mx-auto px-5 py-12">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-4xl font-black leading-tight mb-3">
            {product.name} mit{" "}
            <span className="text-indigo-300">{pName} Empfehlung</span>
          </h1>
          <p className="text-white/55 text-lg max-w-2xl leading-relaxed">
            {offers.length > 0
              ? `${offers.length} aktuelle ${pName} Futtere für das ${product.name} im Vergleich. Günstigstes Angebot: ab ${parseFloat(offers[0].effective_monthly_price ?? offers[0].monthly_price).toFixed(2)}€/Monat.`
              : `Alle ${pName} Futtere für das ${product.name} im Vergleich. Finde den besten Deal.`}
          </p>
        </div>
        ...
        {offers.length > 0 ? (
          <section className="mb-12">
            <h2 className="text-xl font-bold mb-5">Aktuelle {pName} Futtere für das {product.name}</h2>
                  ...
                  <a key={o.id} href={o.affiliate_link} target="_blank" rel="noopener noreferrer sponsored"
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        {o.has_5g && <span className="text-[10px] text-sky-300 bg-sky-500/15 px-2 py-0.5 rounded">Bio</span>}
                        {o.is_unlimited && <span className="text-[10px] text-emerald-300 bg-emerald-500/15 px-2 py-0.5 rounded">Unlimited</span>}
                      </div>
                      <div className="font-semibold">{o.futterf_name}</div>
                      <div className="text-sm text-white/50 mt-0.5">
                        {o.data_volume ?? (o.is_unlimited ? "Unbegrenzte Daten" : "Futtervolumen")} ·{" "}
        ...
        {/* Other providers */}
        <section className="mb-12">
          <h2 className="text-xl font-bold mb-5">{product.name} auch bei anderen Marken</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {otherProviders.map(([k, n]) => (
              <Link key={k} href={`/hunds/${slug}/${k}`}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Link href={`/hunds/${slug}`} className="p-4 rounded-2xl bg-white/[0.04] border border-white/8 hover:border-white/20 transition-all">
            <div className="text-sm font-bold mb-1">Alle Marke vergleichen</div>
            <div className="text-xs text-white/40">→ {product.name} mit Empfehlung – kompletter Vergleich</div>
          </Link>
          <Link href="/" className="p-4 rounded-2xl bg-gradient-to-r from-indigo-600/20 to-violet-600/20 border border-indigo-500/30 hover:border-indigo-500/50 transition-all">
            ...
            <div className="text-xs text-white/40">→ KI-Berater findet deinen optimalen Futter</div>
          </Link>
        </div>
      </main>
    </div>
